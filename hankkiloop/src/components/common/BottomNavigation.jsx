...
export default function BottomNavigation({ onNavigate }) {
  const pathname = location.pathname.startsWith('/fridge/') ? '/fridge' : location.pathname === '/shopping/register' ? (history.state?.source === 'fridge-direct' ? '/fridge' : '/shopping') : location.pathname === '/home' ? '/' : (location.pathname === '/recipes' || location.pathname.startsWith('/recipe/')) ? '/recipe' : location.pathname
