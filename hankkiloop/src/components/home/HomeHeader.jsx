...
export default function HomeHeader({ onProfile, pageLabel = '홈', onNotifications, action }) {
  return (
    <header className="flex h-app-header shrink-0 items-center justify-between border-b border-[#f1f5f9] bg-[#fafcf9] px-5">
      ...
      <div className="flex items-center gap-1">{!action && <NotificationButton onClick={onNotifications} />} {action ?? <button type="button" aria-label="마이페이지 열기" onClick={onProfile} className="flex size-10 items-center justify-center rounded-full"><span className="flex size-8 items-center justify-center rounded-full bg-[#006c49]"><UserRound aria-hidden="true" className="size-3.5 text-white" /></span></button>}</div>
    </header>
  )
}
