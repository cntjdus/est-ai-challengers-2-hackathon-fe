...
export default function AccountInfoCard({ email, nickname, onNicknameChange, realName, isNicknameValid }) {
  return (
    <section aria-labelledby="account-info-heading" className="flex flex-col gap-4 rounded-2xl border border-[#e2e8f0]/80 bg-white p-[17px] shadow-xs">
      ...
      <div>
        ...
        <p className="mt-1 text-[11px] leading-4 text-[#94a3b8]">계정을 바꾸려면 마이페이지에서 로그아웃한 뒤 다시 로그인해주세요.</p>
      </div>
      <div>
        <div className="mb-1.5 flex flex-wrap items-center justify-between gap-1">
          <label htmlFor="edit-nickname" className="text-xs font-semibold text-[#334155]">닉네임</label>
          <span aria-live="polite" className="text-[11px] text-[#047857]">{isNicknameValid ? '✓ 사용할 수 있는 형식' : ''}</span>
        </div>
      ...
      <div>
        <label htmlFor="edit-real-name" className="mb-1.5 block text-xs font-semibold text-[#334155]">Google 계정 이름</label>
        <input id="edit-real-name" value={realName} readOnly className="min-h-10 w-full rounded-xl border border-[#e2e8f0] bg-white px-3 py-2.5 text-xs text-[#1e293b] shadow-xs outline-offset-2 focus:outline-[#047857]" />
      </div>
    </section>
  )
}
