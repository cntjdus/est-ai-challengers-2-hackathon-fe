...
export default function Home({ onNavigate, recommendedRecipes = defaultRecommendedRecipes }) {
  ...
  return (
    <div ref={pageRef} tabIndex={-1} className="mx-auto flex h-dvh w-full max-w-app flex-col overflow-hidden bg-[#fafcf9] outline-none">
      ...
      <main aria-label="홈" className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 pt-[18px] pb-20">
        ...
        <section className="mt-7"><h2 className="mb-3 flex items-center gap-2 px-1 text-sm text-[#1e293b]"><CookingPot aria-hidden="true" className="size-4 text-[#ea8000]" />남은 재료로 오늘 저녁 뚝딱</h2><div className="space-y-3">{recommendedRecipes.map((recipe) => <HomeRecipeCard key={recipe.id} recipe={recipe} onOpen={(id) => onNavigate(`/recipe/${encodeURIComponent(id)}`)} />)}</div></section>
