...
import { calculateExpiryDate, createRegistrationDraft, solutionOptions, storageGuides, validateMaterial } from '../data/materialRegistration'

export default function MaterialRegister({ items, onNavigate, onBack, onRegister }) {
  const [materials, setMaterials] = useState(() => (history.state?.registrationDraft ?? createRegistrationDraft(items)).map((item) => ({ ...item, recognizedFromPhoto: false })))
  ...
  return <div className="mx-auto flex h-dvh w-full max-w-app flex-col overflow-hidden bg-[#fafcf9] text-[#1e293b]">
    <EditProfileHeader title="재료 등록" plain onBack={onBack} action={<button type="button" aria-label="재료 등록 닫기" onClick={() => onNavigate('/shopping')} className="flex size-9 items-center justify-center rounded-full"><X aria-hidden="true" className="size-5" /></button>} />
    <main aria-label="재료 등록" className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 pt-10 pb-6">
      {!material ? <div className="py-12 text-center"><p className="text-sm">등록할 재료가 없습니다.</p><button type="button" onClick={() => onNavigate('/shopping')} className="mt-5 rounded-xl bg-[#006c49] px-5 py-3 text-sm text-white">장바구니로 돌아가기</button></div> : <>
        <aside className="flex items-center gap-3 rounded-2xl border border-[#d1fae5] bg-[#effcf4] p-4"><img src={character} alt="" className="size-12 shrink-0 rounded-2xl object-cover" /><div className="min-w-0"><span className="rounded-full border border-[#d1fae5] bg-white px-2 py-1 text-[10px] text-[#008768]">장바구니 연동</span><p className="mt-1.5 text-xs leading-4 text-[#1b4535]">장바구니에 담은 <strong className="text-[#008768] underline">{material.ingredientName} {material.purchaseAmount}{material.unit}</strong>{materials.length > 1 ? ' 외 ' + (materials.length - 1) + '개 재료를' : '을'} 내 냉장고에 등록할게요!</p></div></aside>
