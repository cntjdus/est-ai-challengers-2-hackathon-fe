...
export default function MaterialRegister({ items, source, onNavigate, onBack, onRegister, onOpenPackageSolution, allowPackageOptions = false }) {
  ...
  const handleMaterialChange = (changes) => {
    ...
    setMaterials((current) => current.map((item, index) => {
      ...
      if ('expiryDate' in changes) next.recognizedFromPhoto = false
  ...
  const handlePhotoChange = (nextPhoto) => {
    setPhoto(nextPhoto)
    if (nextPhoto?.url !== photo?.url) setMaterials((current) => current.map((item, index) => index === activeIndex ? { ...item, recognizedFromPhoto: false, expiryDate: item.expiryAutomatic ? '' : item.expiryDate, expiryAutomatic: false } : item))
  }
  ...
  return <div className="mx-auto flex h-dvh w-full max-w-app flex-col overflow-hidden bg-[#fafcf9] text-[#1e293b]">
    ...
    <main aria-label="재료 등록" className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 pt-10 pb-6">
      {!material ? <div className="py-12 text-center"><p className="text-sm">등록할 재료가 없습니다.</p><button type="button" onClick={() => onNavigate(source === 'fridge-direct' ? '/fridge' : '/shopping')} className="mt-5 rounded-xl bg-[#006c49] px-5 py-3 text-sm text-white">장바구니로 돌아가기</button></div> : <>
        ...
        <div hidden={mode !== 'photo'}><PhotoRecognitionSection key={material.id} photo={photo} onPhotoChange={handlePhotoChange} materialName={material.ingredientName} onConfirm={({ date, kind }) => {
          if (kind !== 'use_by') return
          setMaterials(current => current.map(item => item.id === material.id ? { ...item, expiryDate: date, expiryAutomatic: false, recognizedFromPhoto: true } : item))
          setError('')
        }} /></div>
        {(materials.length > 1 || mode === 'photo') && <section className="my-5"><h2 className="text-sm">{mode === 'photo' ? '날짜를 확인한 재료' : '등록할 식재료'} <span className="rounded-full bg-[#e2efe9] px-1.5 text-xs text-[#006c49]">{mode === 'photo' ? recognizedIngredients.length : materials.length}</span></h2><div className="mt-2 flex flex-wrap gap-2">{materials.map((item, index) => <button type="button" key={item.id} aria-pressed={index === activeIndex} onClick={() => { if (index !== activeIndex) setPhoto(null); setActiveIndex(index); setError('') }} className="rounded-full border border-[#e2e8f0] px-3 py-1.5 text-xs text-[#64748b] aria-pressed:border-[#10b981] aria-pressed:bg-[#ecfdf5] aria-pressed:text-[#006c49]">{item.ingredientName || '이름 없음'}</button>)}</div></section>}
