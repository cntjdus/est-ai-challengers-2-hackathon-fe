...
it('shows exploration only on the recipe tab and searches the existing catalog', async () => {
  history.replaceState({}, '', '/recipe')
  mount()
  expect(await screen.findByRole('heading', { name: 'DB 계란 요리' })).toBeTruthy()
  expect(screen.getByRole('search')).toBeTruthy()
  expect(screen.getByText('예상 식비 절약 & 소진 효과')).toBeTruthy()
  fireEvent.change(screen.getByRole('textbox', { name: '레시피 검색' }), { target: { value: '없는 재료' } })
  fireEvent.click(screen.getByRole('button', { name: '검색' }))
  expect(screen.queryByRole('heading', { name: 'DB 계란 요리' })).toBeNull()
  fireEvent.click(screen.getByRole('button', { name: /스크랩/ }))
  expect(screen.queryByRole('search')).toBeNull()
  expect(screen.queryByText('예상 식비 절약 & 소진 효과')).toBeNull()
  fireEvent.click(screen.getAllByRole('button', { name: '레시피' })[0])
  expect(screen.getByRole('search')).toBeTruthy()
  expect(screen.getByRole('heading', { name: 'DB 계란 요리' })).toBeTruthy()
})
